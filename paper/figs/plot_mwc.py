...
FITS_PATH = 'MWC_758_M4368_2024-12-31_02310.fits'
ORDER = '04'
CHIPS = [1, 2, 3]
...
for ax, chip in zip(axes, CHIPS):
    d = fi[chip].data
    w = d[f'{ORDER}_01_WL']
    s = d[f'{ORDER}_01_SPEC']   # telluric-corrected = original / TELLUR
    c = d[f'{ORDER}_01_CONT']
    t = d[f'{ORDER}_01_TELLUR']

    observed = s * t        # original pre-correction spectrum
    model = c * t           # full vipere model (continuum * tellurics)
    corrected = np.where(t > 0.3, s, np.nan)  # OBS / TELL (masked in deep cores)
...
axes[0].set_title(r'MWC 758 — setting M4368 — order 04')
axes[0].legend(loc='lower left', framealpha=0, fontsize=8)
axes[-1].set_xlabel(r'Wavelength [$\mu$m]')

fig.subplots_adjust(hspace=0.18)

fig.savefig('mwc758_CO.pdf', bbox_inches='tight')
fig.savefig('mwc758_CO.png', bbox_inches='tight', dpi=150)
